...
This document covers everything you need to integrate Remote Agent Control into your own application: host interface specifications, SSE stream format, step-by-step integration walkthrough, adapter development, and troubleshooting.
...
Remote Agent Control Core uses dependency injection via a global context. Your application must:
...
Your Application
    ...
    |
    v
Remote Agent Control Core (handles IM ↔ LLM message flow automatically)
```
...
interface ChannelBinding {
  ...
  model: string;
  mode: 'code' | 'plan' | 'ask';
  /** Sandbox permission cap — bounds the tool capabilities the backend may use. */
...
```bash
# From the source workspace root
npm run install:all
npm run build

# Or work inside the core library package
cd Claude-to-IM
npm install
