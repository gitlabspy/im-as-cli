...
> 状态：**待开始**
> 目标：为 remote-agent-control 增加飞书 V2 流式卡片能力

## 背景

目标实现：CardKit v2 流式卡片、工具进度、权限内联按钮、Thinking 状态、Footer 耗时。
remote-agent-control 当前飞书实现：无流式（发最终结果）、权限靠文本命令 `/perm`、无工具进度显示。
...
理由：
- remote-agent-control 是轻量库，没有独立的 channel plugin 架构
- 卡片状态（cardId、sequence、throttle）与 adapter 生命周期绑定
- 保持一个文件内聚，降低理解成本

## 文件改动清单

### remote-agent-control 仓库
