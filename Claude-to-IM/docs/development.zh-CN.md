...
本文档涵盖将 Remote Agent Control 集成到你自己应用中所需的全部内容：宿主接口规范、SSE 流格式、分步集成教程、适配器开发指南和故障排查。
...
Remote Agent Control 通过全局上下文实现依赖注入。你的应用需要：
...
你的应用
    ...
    |
    v
Remote Agent Control Core（自动处理 IM <-> LLM 消息流）
```
