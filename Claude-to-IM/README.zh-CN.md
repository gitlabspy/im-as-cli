...
Remote Agent Control Core 是一个与宿主应用解耦的桥接库，用于将 [Claude Code SDK](https://docs.anthropic.com/en/docs/claude-code/sdk) 连接到 IM 平台，让用户可以通过 Telegram、Discord 和飞书直接与 Claude 交互。

这个库处理了 IM 侧的所有复杂性——消息路由、流式预览、权限审批流程、Markdown 渲染、消息分块、重试、速率限制——同时通过一组依赖注入接口将持久化、LLM 调用和权限解析委托给宿主应用。

如果你想直接把 Claude Code/Codex 接到 IM，请使用同一源码工作区里的
`Claude-to-IM-skill/` 包。这个包是核心桥接库，供需要在自己的 Node.js
宿主应用中嵌入桥接能力的开发者使用。
...
为了兼容已有宿主（例如同级的 `Claude-to-IM-skill/` 包），包里也保留了
source-style 子路径：
...
这两种写法都会通过 `package.json` 的 `exports` map 解析到编译后的 `dist/`
文件；运行时不会导入 TypeScript 源码。
...
- **修复飞书流式卡片。** 飞书 adapter 此前调用的 `cardkit.v2.*` API 命名空间在 `@larksuiteoapi/node-sdk` 中**根本不存在**（在 1.59 和 1.66 两个版本都已验证——SDK 只提供 `cardkit.v1`）。这导致创建卡片时直接抛错，所以用户只能看到一条最终回复、看不到实时进度。已将四处调用全部改写为真实的 `cardkit.v1` API：`card.create`、`cardElement.content`（增量打字机更新）、`card.settings`（以 JSON 字符串传入 settings 关闭流式）、`card.update`（card 嵌套在 `data.card` 下，`sequence` 严格递增）。
- **最终回复改为真正的卡片。** 最终回复现在带有真实的卡片标题（`title` = `✅ 最后答复` 标记，颜色按状态区分）和真实的 `note` 脚注元素（状态 · 耗时 / token / 模型），不再是内联到正文的文本。
