...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  /**
   * Reset per-chat UI state for `/repair`. Clears typing reaction tracking
   * and last incoming message id. Bridge-manager owns persistent binding state.
   */
  resetChatState(chatId: string): void {
    this.typingReactions.delete(chatId);
    this.lastIncomingMessageId.delete(chatId);
  }
  ...
  async sendInteractiveCard(
    ...
    try {
      const res = await this.sendOrReply(address.chatId, 'interactive', cardJson, replyToMessageId);
      if (res?.data?.message_id) {
        return { ok: true, messageId: res.data.message_id };
      }
      return { ok: false, error: res?.msg || 'Interactive card send failed' };
    } catch (err) {
      return { ok: false, error: err instanceof Error ? err.message : 'Interactive card send failed' };
    }
  }

  /**
   * Send a payload, threading via `im.message.reply` when `replyToMessageId`
   * is provided. Returns the underlying client response (or throws).
   */
  private async sendOrReply(
    ...
    content: string,
    replyToMessageId?: string,
  ): Promise<{ data?: { message_id?: string }; code?: number; msg?: string } | undefined> {
    if (replyToMessageId) {
      try {
        return await this.restClient!.im.message.reply({
          path: { message_id: replyToMessageId },
          data: { msg_type: msgType, content },
        });
      } catch (err) {
        console.warn('[feishu-adapter] Reply failed, falling back to create:', err instanceof Error ? err.message : err);
        // Fall through to create
      }
    }
    return await this.restClient!.im.message.create({
  ...
  private async sendAsCard(chatId: string, text: string, replyToMessageId?: string): Promise<SendResult> {
    ...
    try {
      const res = await this.sendOrReply(chatId, 'interactive', cardContent, replyToMessageId);
  ...
  private async sendAsPost(chatId: string, text: string, replyToMessageId?: string): Promise<SendResult> {
    ...
    try {
      const res = await this.sendOrReply(chatId, 'post', postContent, replyToMessageId);
    ...
    try {
      const res = await this.sendOrReply(chatId, 'text', JSON.stringify({ text }), replyToMessageId);
      if (res?.data?.message_id) {
        return { ok: true, messageId: res.data.message_id };
      }
