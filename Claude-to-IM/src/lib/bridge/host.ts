 ...
 * (for example, a desktop app, CLI daemon, or service). A host must provide implementations of these
 * interfaces to use the bridge.
 */

import type { BackendName, BridgeSessionTab, ChannelBinding, ChannelType, OutputVerbosity, SandboxLevel } from './types.js';
...
export interface BridgeSession {
  ...
  provider_id?: string;
  /**
   * Provider-level resume id (Claude Code session id / Codex thread id /
   * Copilot session id). This is the authoritative location for the resume
   * id — `ChannelBinding.sdkSessionId` is just a read-through cache that
   * mirrors this value for the currently-active session.
   */
  sdkSessionId?: string;
}
...
export interface UpsertChannelBindingInput {
  ...
  model: string;
  mode?: string;
  // ── Optional backend-switch fields (Phase 1) ──
  backend?: BackendName;
...
export interface StreamChatParams {
  ...
  // ── Backend routing (Phase 2) ──
  /** Selected backend name for this stream; provider router will dispatch accordingly. */
  backend?: BackendName;
  /** Per-backend native SDK/CLI session ID for resume across turns. */
  backendSdkSessionId?: string;
  /** Sandbox capability cap for this turn ('ro' | 'rw' | 'full'). Defaults to 'rw' if absent. */
  sandboxLevel?: SandboxLevel;
  /** One-shot helper call that must not reuse or cache native provider sessions. */
  ephemeral?: boolean;
}
...
// ── Relay events (Phase 4) ────────────────────────────────────
//
// Typed events emitted by the conversation engine for adapters/bridge-manager
// to throttle, aggregate, and deliver per `OutputVerbosity` policy.
export type RelayEvent =
  | { kind: 'status'; text: string; backend?: BackendName }
  | { kind: 'tool'; toolId: string; name: string; status: 'running' | 'complete' | 'error'; summary?: string }
...
export interface PermissionGateway {
   ...
   */
  resolvePendingPermission(permissionRequestId: string, resolution: PermissionResolution): boolean | Promise<boolean>;
}
...
export interface LifecycleHooks {
  ...
  onBridgeStop?(): void;
  /** Handle host-level control commands such as /ctl status, /ctl logs, /ctl restart. */
  handleControlCommand?(request: ControlCommandRequest): Promise<ControlCommandResponse> | ControlCommandResponse;
}
