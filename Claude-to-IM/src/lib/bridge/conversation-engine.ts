...
/**
 * Callback invoked for typed relay events (Phase 4). Bridge-manager handles
 * per-binding throttling/aggregation based on `outputVerbosity` before
 * delivering to the adapter.
 */
export type OnRelayEvent = (event: RelayEvent) => void;
...
export async function processMessage(
  ...
  try {
    ...
    // Permission mode from binding mode, overridden by sandbox level.
    // Sandbox 'ro' / 'full' force CC permissionMode regardless of /mode.
    const sandboxLevel = binding.sandboxLevel ?? 'rw';
    let permissionMode: string;
    if (sandboxLevel === 'ro') {
      permissionMode = 'plan';
    } else if (sandboxLevel === 'full') {
    ...
    // NOTE: Do NOT inject the bridge's local message log as conversationHistory.
    // The provider's own resume mechanism (sdkSessionId / backendSdkSessionId) is
    // the single source of truth for conversation context. The local message log
    // exists for audit / UI / debug only — re-injecting it would contaminate the
    // backend session with the bridge's view of history.
    ...
    const effectiveBackend = binding.backend;
    const effectiveBackendSdkSessionId =
      effectiveBackend && binding.backendSdkSessionIds
        ? binding.backendSdkSessionIds[effectiveBackend]
        : undefined;
    ...
    const stream = llm.streamChat({
      ...
      backendSdkSessionId: effectiveBackendSdkSessionId,
      sandboxLevel,
    ...
    return await consumeStream(stream, sessionId, onPermissionRequest, onPartialText, onToolEvent, onRelayEvent, binding.backend);
  } finally {
...
async function consumeStream(
  ...
  try {
        ...
        switch (event.type) {
          ...
          case 'status': {
            try {
              ...
              if (onRelayEvent && typeof statusData.text === 'string' && statusData.text) {
                try { onRelayEvent({ kind: 'status', text: statusData.text, backend }); } catch { /* non-critical */ }
              }
            } catch { /* skip */ }
            break;
          }
          ...
          case 'task_update': {
            try {
              ...
              if (onRelayEvent && Array.isArray(taskData.todos)) {
                const inProgress = taskData.todos.find((t: { status?: string; content?: string }) => t.status === 'in_progress');
                if (inProgress && typeof inProgress.content === 'string') {
                  try { onRelayEvent({ kind: 'task', text: inProgress.content }); } catch { /* non-critical */ }
                }
              }
            } catch { /* skip */ }
            break;
          ...
          case 'error':
            ...
            if (onRelayEvent) {
              try { onRelayEvent({ kind: 'error', text: errorMessage }); } catch { /* non-critical */ }
            }
            break;
    ...
    if (onRelayEvent && responseText) {
      try { onRelayEvent({ kind: 'final', text: responseText }); } catch { /* non-critical */ }
    }
