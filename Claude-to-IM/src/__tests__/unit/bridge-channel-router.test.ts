...
function createMockStore(): BridgeStore & { bindings: Map<string, ChannelBinding>; sessions: Map<string, { id: string; working_directory: string; model: string; sdkSessionId?: string }> } {
  const bindings = new Map<string, ChannelBinding>();
  const sessions = new Map<string, { id: string; working_directory: string; model: string; sdkSessionId?: string }>();
  let nextId = 1;
  ...
  return {
    ...
    upsertChannelBinding(data) {
      ...
      const hasSdk = Object.prototype.hasOwnProperty.call(data, 'sdkSessionId');
      const binding: ChannelBinding = existing
        ? {
            ...existing,
            codepilotSessionId: data.codepilotSessionId,
            sdkSessionId: hasSdk ? (data.sdkSessionId ?? '') : existing.sdkSessionId,
...
describe('channel-router', () => {
  ...
  it('bindToSession() carries the target session sdkSessionId into the binding', () => {
    // Simulate a session that has already produced a provider-level resume id.
    const session = store.createSession('Existing', 'claude-3', undefined, '/work');
    (session as { sdkSessionId?: string }).sdkSessionId = 'sdk-target-xyz';

    // Pre-existing binding for this Feishu chat carries some other resume id.
    const previous = router.resolve({ channelType: 'telegram', chatId: 'chatX' });
    router.updateBinding(previous.id, { sdkSessionId: 'sdk-previous' });
    assert.equal(store.bindings.get('telegram:chatX')?.sdkSessionId, 'sdk-previous');

    // /bind to the target session — binding's cached sdkSessionId MUST refresh.
    const rebound = router.bindToSession(
      { channelType: 'telegram', chatId: 'chatX' },
      session.id,
    );
    assert.ok(rebound);
    assert.equal(rebound!.codepilotSessionId, session.id);
    assert.equal(rebound!.sdkSessionId, 'sdk-target-xyz');
  });
});
