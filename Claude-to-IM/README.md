# Remote Agent Control Core

English | [中文](README.zh-CN.md)

Remote Agent Control Core is a host-agnostic bridge library that connects local
coding agents to IM platforms, allowing users to interact through Telegram,
Discord, Feishu/Lark, QQ, and WeChat.
...
For a ready-to-use Claude Code/Codex bridge, use the sibling
`Claude-to-IM-skill/` package in this source workspace. This package is the core
library for developers who want to embed the bridge in their own Node.js host.
...
When working from the source workspace, install and build from the repository
root:
...
```typescript
import { initBridgeContext } from 'remote-agent-control-core/context';
import type { BridgeStore, LLMProvider, PermissionGateway, LifecycleHooks } from 'remote-agent-control-core/host';
...
```typescript
import * as bridgeManager from 'remote-agent-control-core/bridge-manager';
...
Host applications should use the short public subpaths:
...
The package also keeps source-style subpaths for existing hosts such as the
sibling `Claude-to-IM-skill/` package:
...
Both styles resolve to the compiled `dist/` files through the `exports` map in
`package.json`; consumers never import the TypeScript source at runtime.

**Contract for library maintainers:** this `exports` map is load-bearing. Hosts keep the
library external (un-bundled) and load it from `dist/` at runtime, so:

- Always `npm run build` after changing `src/lib/` — hosts run the **compiled** output.
- If you add a new public subpath, extend the `exports` map or hosts can't import it.
- Don't rename/remove an exported subpath without a coordinated host update.

A rebuild here is picked up by the host on its next daemon restart — no host rebundle
needed. See the host's `DEVELOPING.md` for the full build chain.
...
- **Fixed Feishu streaming cards.** The Feishu adapter was calling a `cardkit.v2.*` API namespace that **does not exist** in `@larksuiteoapi/node-sdk` (verified at both 1.59 and 1.66 — only `cardkit.v1` ships). This threw on card creation, so users only ever saw a single final reply with no live progress. Rewrote all four call sites to the real `cardkit.v1` API: `card.create`, `cardElement.content` (incremental typewriter updates), `card.settings` (JSON-encoded settings payload to close streaming), and `card.update` (card nested under `data.card`, strictly incrementing `sequence`).
- **Final reply is now a proper card.** The final reply renders with a real card header (`title` = the `✅ 最后答复` marker, color band by status) and a real `note` footnote element (status · elapsed / tokens / model) instead of inlined body text.
