...
describe('CodexProvider', () => {
  ...
  // ── sandboxLevel → sandboxMode mapping ──────────────────────
  async function runWithSandbox(level: 'ro' | 'rw' | 'full' | undefined): Promise<Record<string, unknown> | undefined> {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    const stream = provider.streamChat({
      prompt: 'sandbox test',
      sessionId: `sandbox-${level ?? 'default'}-session`,
      ...(level ? { sandboxLevel: level } : {}),
    } as any);
    await collectStream(stream);
    return capturedStartOptions;
  }

  it('maps sandboxLevel="ro" to sandboxMode="read-only"', async () => {
    const opts = await runWithSandbox('ro');
    assert.equal(opts?.sandboxMode, 'read-only');
  });

  it('maps sandboxLevel="rw" to sandboxMode="workspace-write"', async () => {
    const opts = await runWithSandbox('rw');
    assert.equal(opts?.sandboxMode, 'workspace-write');
  });

  it('maps sandboxLevel="full" to sandboxMode="danger-full-access"', async () => {
    const opts = await runWithSandbox('full');
    assert.equal(opts?.sandboxMode, 'danger-full-access');
  });

  it('defaults to sandboxMode="workspace-write" when sandboxLevel is omitted', async () => {
    const opts = await runWithSandbox(undefined);
    assert.equal(opts?.sandboxMode, 'workspace-write');
  });
});
