...
describe('handleMessage state tracking', () => {
  it('uses Claude user/project/local setting sources by default', () => {
    assert.deepEqual([...DEFAULT_SETTING_SOURCES], ['user', 'project', 'local']);
  });
  ...
  it('emits result text as an error when success result has is_error=true', () => {
    const { controller, chunks } = makeFakeController();
    const state = freshState();
    ...
    assert.equal(state.hasReceivedResult, true);
    const events = chunks.map(c => JSON.parse(c.replace('data: ', '')));
    assert.equal(events[0].type, 'error');
    assert.equal(events[0].data, 'Not logged in · Please run /login');
