...
describe('JsonFileStore', () => {
  ...
  it('upsertChannelBinding clears backend sdk ids when explicitly reset', () => {
    const store = new JsonFileStore(makeSettings());
    store.upsertChannelBinding({
      channelType: 'feishu',
      chatId: 'chat-1',
      codepilotSessionId: 'sess-old',
      sdkSessionId: 'sdk-old',
      workingDirectory: '/tmp',
      model: 'model-1',
      backend: 'codex',
      backendSdkSessionIds: { codex: 'sdk-old' },
    });

    const updated = store.upsertChannelBinding({
      channelType: 'feishu',
      chatId: 'chat-1',
      codepilotSessionId: 'sess-new',
      sdkSessionId: '',
      workingDirectory: '/tmp',
      model: 'model-1',
      backend: 'codex',
    ...
    assert.deepEqual(updated.backendSdkSessionIds, {});
  });
