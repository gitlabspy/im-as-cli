 ...
 * the Remote Agent Control bridge conversation engine.
 */
...
import type { LLMProvider, StreamChatParams, FileAttachment } from 'remote-agent-control-core/src/lib/bridge/host.js';
...
/** Claude settings sources needed for OAuth/keychain auth and normal CLI config. */
export const DEFAULT_SETTING_SOURCES = ['user', 'project', 'local'] as const;
          ...
          try {
            ...
            if (authKind === 'cli') {
              ...
              lines.push(
                ...
                '',
                'Run `/remote-agent-control doctor` to diagnose.',
              );
      ...
      if (msg.subtype === 'success') {
        if (msg.is_error) {
          const resultText = typeof msg.result === 'string' && msg.result.trim()
            ? msg.result
            : 'Unknown error';
          controller.enqueue(sseEvent('error', resultText));
        }
        controller.enqueue(
          sseEvent('result', {
