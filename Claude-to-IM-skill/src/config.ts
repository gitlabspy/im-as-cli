...
export interface Config {
  runtime: 'claude' | 'codex' | 'copilot' | 'auto';
  workerMode: 'managed' | 'embedded';
  ...
  autoApprove?: boolean;
  // Host-level /ctl authorization.
  controlAllowedUsers?: string[];
  controlAllowedChats?: string[];
}
...
export function loadConfig(): Config {
  ...
  const runtime = (["claude", "codex", "copilot", "auto"].includes(rawRuntime) ? rawRuntime : "claude") as Config["runtime"];
  const rawWorkerMode = env.get("CTI_WORKER_MODE") || "managed";
  const workerMode = (["managed", "embedded"].includes(rawWorkerMode) ? rawWorkerMode : "managed") as Config["workerMode"];

  return {
    runtime,
    workerMode,
    workerAutoStart: env.get("CTI_WORKER_AUTO_START") !== "false",
    ...
    controlAllowedUsers: splitCsv(env.get("CTI_CTL_ALLOWED_USERS")),
    controlAllowedChats: splitCsv(env.get("CTI_CTL_ALLOWED_CHATS")),
  };
}

export function loadConfigEnvIntoProcess(): void {
...
export function saveConfig(config: Config): void {
  ...
  out += formatEnvLine("CTI_WORKER_MODE", config.workerMode);
  out += formatEnvLine("CTI_WORKER_AUTO_START", String(config.workerAutoStart));
  out += formatEnvLine(
  ...
  out += formatEnvLine("CTI_CTL_ALLOWED_USERS", config.controlAllowedUsers?.join(","));
  out += formatEnvLine("CTI_CTL_ALLOWED_CHATS", config.controlAllowedChats?.join(","));
...
export function configToSettings(config: Config): Map<string, string> {
  ...
  // ── Backend ──
  // Upstream key: bridge_default_backend (claudecode | codex | copilot)
  const backendMap: Record<Config['runtime'], string> = {
    claude: 'claudecode',
    codex: 'codex',
    copilot: 'copilot',
    auto: 'claudecode',
  };
  m.set('bridge_default_backend', backendMap[config.runtime]);
  ...
  m.set("bridge_worker_mode", config.workerMode);
  m.set("bridge_worker_auto_start", String(config.workerAutoStart));
