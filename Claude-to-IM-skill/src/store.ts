...
} from 'remote-agent-control-core/src/lib/bridge/host.js';
import type { BackendName, ChannelBinding, ChannelType } from 'remote-agent-control-core/src/lib/bridge/types.js';
...
export class JsonFileStore implements BridgeStore {
  ...
  private loadAll(): void {
    ...
    // Bindings — backfill backend fields on legacy records.
    const bindings = readJson<Record<string, ChannelBinding>>(
      path.join(DATA_DIR, 'bindings.json'),
      {},
    );
    const envBackend = (process.env.CTI_RUNTIME || '').toLowerCase();
    const settingBackend = (this.settings.get('bridge_default_backend') || '').toLowerCase();
    const fallbackBackend: BackendName =
      (settingBackend === 'codex' || settingBackend === 'copilot' || settingBackend === 'claudecode')
        ? (settingBackend as BackendName)
        : envBackend === 'codex' ? 'codex'
        : envBackend === 'copilot' ? 'copilot'
        : 'claudecode';
    for (const [key, raw] of Object.entries(bindings)) {
      const b = raw as ChannelBinding;
      if (!b.backend) {
        b.backend = fallbackBackend;
      }
      if (b.backendGeneration === undefined) {
        b.backendGeneration = 0;
      }
      if (!b.backendSessionIds) {
        b.backendSessionIds = { [b.backend]: b.codepilotSessionId } as Partial<Record<BackendName, string>>;
      }
      if (!b.backendSdkSessionIds) {
        b.backendSdkSessionIds = b.sdkSessionId
          ? ({ [b.backend]: b.sdkSessionId } as Partial<Record<BackendName, string>>)
          : {};
      }
      if (!b.outputVerbosity) {
        b.outputVerbosity = 'normal';
      }
      if (!b.sessionTabs || b.sessionTabs.length === 0) {
        b.sessionTabs = [{
          id: b.codepilotSessionId,
          codepilotSessionId: b.codepilotSessionId,
          ...
          backendSessionIds: b.backendSessionIds,
          backendSdkSessionIds: b.backendSdkSessionIds,
          outputVerbosity: b.outputVerbosity,
          sandboxLevel: b.sandboxLevel ?? 'rw',
          status: 'idle',
          unread: false,
      ...
      if (!b.activeSessionTabId) {
        b.activeSessionTabId = b.sessionTabs.find((t) => t.codepilotSessionId === b.codepilotSessionId)?.id
          ?? b.sessionTabs[0]?.id
          ?? b.codepilotSessionId;
      }
  ...
  upsertChannelBinding(data: UpsertChannelBindingInput): ChannelBinding {
    ...
    if (existing) {
      // Honor sdkSessionId only when the caller explicitly provided it
      // (including the empty string, which means "reset"). Otherwise the
      // existing cached value is preserved.
      const hasSdkSessionId = Object.prototype.hasOwnProperty.call(
        data,
        'sdkSessionId',
      );
      const hasBackendSdkSessionIds = Object.prototype.hasOwnProperty.call(
        data,
        'backendSdkSessionIds',
      );
      const hasSessionTabs = Object.prototype.hasOwnProperty.call(
        data,
        'sessionTabs',
      );
      const updated: ChannelBinding = {
        ...
        sdkSessionId: hasSdkSessionId
          ? data.sdkSessionId ?? ''
          : existing.sdkSessionId,
        workingDirectory: data.workingDirectory,
        model: data.model,
        // ── Preserve / merge backend fields ──
        backend: data.backend ?? existing.backend,
        backendGeneration: data.backendGeneration ?? existing.backendGeneration,
        backendSessionIds: data.backendSessionIds
          ? { ...(existing.backendSessionIds ?? {}), ...data.backendSessionIds }
          : existing.backendSessionIds,
        backendSdkSessionIds: hasBackendSdkSessionIds
          ? { ...data.backendSdkSessionIds }
          : existing.backendSdkSessionIds,
        outputVerbosity: data.outputVerbosity ?? existing.outputVerbosity,
        sandboxLevel: data.sandboxLevel ?? existing.sandboxLevel,
        sessionTabs: hasSessionTabs
          ? data.sessionTabs
          : existing.sessionTabs,
        activeSessionTabId: data.activeSessionTabId ?? existing.activeSessionTabId,
        updatedAt: now(),
      };
    ...
    const defaultBackend: BackendName =
      data.backend
      ?? ((this.settings.get('bridge_default_backend')?.toLowerCase() as BackendName | undefined))
      ?? 'claudecode';
    const binding: ChannelBinding = {
      ...
      backendGeneration: data.backendGeneration ?? 0,
      backendSessionIds:
        data.backendSessionIds
        ?? ({ [defaultBackend]: data.codepilotSessionId } as Partial<Record<BackendName, string>>),
      backendSdkSessionIds: data.backendSdkSessionIds ?? {},
      outputVerbosity: data.outputVerbosity ?? 'normal',
      sandboxLevel: data.sandboxLevel ?? 'rw',
      sessionTabs: data.sessionTabs,
      activeSessionTabId: data.activeSessionTabId ?? data.codepilotSessionId,
    };
  ...
  getSession(id: string): BridgeSession | null {
    ...
    if (!s) return null;
    const rawSdk = (s as unknown as Record<string, unknown>)['sdk_session_id'];
    if (typeof rawSdk === 'string') {
      return { ...s, sdkSessionId: rawSdk };
    }
    return { ...s };
  }
  ...
  updateSdkSessionId(sessionId: string, sdkSessionId: string): void {
    ...
    // Update any binding whose currently-active backend lane points to this session.
    // We avoid clobbering other backends' lanes by only writing when codepilotSessionId
    // matches (i.e., this session is the active lane for that binding).
    for (const [key, b] of this.bindings) {
      if (b.codepilotSessionId === sessionId) {
        const lane = b.backend;
        const nextSdkLane: Partial<Record<BackendName, string>> = {
          ...(b.backendSdkSessionIds ?? {}),
        };
        if (lane) nextSdkLane[lane] = sdkSessionId;
        this.bindings.set(key, {
          ...b,
          sdkSessionId,
          backendSdkSessionIds: nextSdkLane,
          updatedAt: now(),
        });
      }
    }
