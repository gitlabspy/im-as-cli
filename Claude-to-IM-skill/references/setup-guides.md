...
Run `/remote-agent-control start` in Claude Code. This establishes the WebSocket long connection that Feishu needs to detect.
...
3. **Start (or restart) the bridge** — Run `/remote-agent-control start` so the WebSocket connection is active
...
6. **Restart the bridge** — Run `/remote-agent-control stop` then `/remote-agent-control start` to pick up the new capabilities
...
> Risk note: this integration follows an OpenClaw-style WeChat plugin protocol. Because it connects a non-OpenClaw product to WeChat, there may be account risk. Use with caution.
...
```bash
cd ~/.claude/skills/remote-agent-control
...
```bash
cd ~/.codex/skills/remote-agent-control
npm run weixin:login
```
