...
**Symptoms**: `/remote-agent-control start` fails or daemon exits immediately.

**Steps**:

1. Run `/remote-agent-control doctor` to identify the issue
...
5. Check logs for startup errors: `/remote-agent-control logs`

**Common causes**:
- Missing or invalid config.env -- run `/remote-agent-control setup`
- Node.js not found or wrong version -- install Node.js >= 20
- Port or resource conflict -- check if another instance is running with `/remote-agent-control status`
...
1. Verify the bot token is valid: `/remote-agent-control doctor`
...
6. Check logs for incoming message events: `/remote-agent-control logs 200`
...
1. Check current memory usage: `/remote-agent-control status`
2. Restart the daemon to reset memory:
   ```
   /remote-agent-control stop
...
1. Run `/remote-agent-control stop` -- it will clean up the stale PID file
2. If stop also fails, manually remove the PID file:
   ```bash
   rm ~/.claude-to-im/runtime/bridge.pid
   ```
3. Run `/remote-agent-control start` to launch a fresh instance
