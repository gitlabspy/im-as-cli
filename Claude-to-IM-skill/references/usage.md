...
This skill works with both **Claude Code** (via `/remote-agent-control` slash commands) and **Codex** (via natural language like "start bridge", "配置", "诊断"). All commands below use Claude Code syntax; Codex users can use equivalent natural language.
...
If startup fails, run `/remote-agent-control doctor` to diagnose issues.
...
```
/remote-agent-control logs        # Last 50 lines (default)
/remote-agent-control logs 200    # Last 200 lines
```
...
```
/remote-agent-control reconfigure
```
