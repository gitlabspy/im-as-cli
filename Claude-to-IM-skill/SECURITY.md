...
3. Run `/remote-agent-control reconfigure` to update the stored credentials
4. Run `/remote-agent-control stop` then `/remote-agent-control start` to apply changes
...
2. Run `/remote-agent-control stop` to halt the daemon
3. Run `/remote-agent-control reconfigure` with a new token
4. Review `~/.claude-to-im/logs/` for unauthorized activity
5. Run `/remote-agent-control start` with the new credentials
