...
description: |
  ...
  or diagnosing the Remote Agent Control bridge daemon; forwarding agent replies to a messaging
  app; any phrase like "remote-agent-control", "bridge", "消息推送", "消息转发", "桥接",
...
You are managing the Remote Agent Control bridge.
User data is stored at `~/.claude-to-im/`.

The skill directory (SKILL_DIR) is usually at `~/.claude/skills/remote-agent-control`
or `~/.codex/skills/remote-agent-control`. These paths may be junctions/symlinks to a
separate development checkout. If neither path exists, fall back to Glob with
pattern `**/skills/**/remote-agent-control/SKILL.md` and derive the root from the result.
...
- **Feishu**: App ID → confirm → App Secret → confirm (masked) → Domain (optional) → Allowed User IDs (optional). After collecting credentials, explain the two-phase setup the user must complete:
  ...
  - **Phase 2** (requires running bridge): (D) run `/remote-agent-control start`, (E) configure events (`im.message.receive_v1`) and callback (`card.action.trigger`) with long connection mode, (F) publish second version + admin approve.
...
8. On success, tell the user: "Setup complete! Run `/remote-agent-control start` to start the bridge."
...
- Run `doctor` to diagnose: `/remote-agent-control doctor`
- Check recent logs: `/remote-agent-control logs`
...
7. Remind user: "Run `/remote-agent-control stop` then `/remote-agent-control start` to apply the changes."
