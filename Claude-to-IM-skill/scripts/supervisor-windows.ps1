<#
.SYNOPSIS
  Windows daemon manager for Remote Agent Control bridge.
...
.DESCRIPTION
  ...
  Usage:
    ...
    powershell -File scripts\daemon.ps1 restart
...
param(
    ...
    [ValidateSet('start','stop','restart','status','logs','install-service','uninstall-service','help')]
    [string]$Command = 'help',
...
function Invoke-NpmBuild {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "npm run build failed in $Directory (exit code $LASTEXITCODE)"
        }
    } finally {
        Pop-Location
    }
...
function Get-NewestBuildInput {
    ...
    if (Test-Path -LiteralPath $srcDir -PathType Container) {
        $inputs += Get-ChildItem -LiteralPath $srcDir -Recurse -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match '\.(ts|tsx|mts|cts)$' }
    }
    ...
    return $inputs | Sort-Object LastWriteTime -Descending | Select-Object -First 1
}

function Get-NewestDistFile {
    ...
    return Get-ChildItem -LiteralPath $DistDir -Recurse -File -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
}

function Ensure-CoreBuilt {
    $coreDir = Join-Path (Join-Path $SkillDir 'node_modules') 'remote-agent-control-core'
    $corePackageJson = Join-Path $coreDir 'package.json'
    if (-not (Test-Path -LiteralPath $corePackageJson -PathType Leaf)) {
        return $null
    }

    $coreDistDir = Join-Path $coreDir 'dist'
    $coreDistNewest = Get-NewestDistFile $coreDistDir
    $coreNewestInput = Get-NewestBuildInput -BaseDir $coreDir -MetadataFiles @(
        'scripts/build.js',
        'package.json',
        'package-lock.json',
        'tsconfig.json',
    ...
    if (-not $coreDistNewest) {
        Write-Host "Building Remote Agent Control core (dist missing)..."
        Invoke-NpmBuild -Directory $coreDir
        return Get-NewestDistFile $coreDistDir
    }

    if ($coreNewestInput -and $coreNewestInput.LastWriteTime -gt $coreDistNewest.LastWriteTime) {
        Write-Host "Rebuilding Remote Agent Control core (input changed: $($coreNewestInput.FullName))..."
        Invoke-NpmBuild -Directory $coreDir
        return Get-NewestDistFile $coreDistDir
    }

    return $coreDistNewest
}

function Ensure-Built {
    $coreDistNewest = Ensure-CoreBuilt

    if (-not (Test-Path -LiteralPath $DaemonMjs -PathType Leaf)) {
        Write-Host "Building daemon bundle (dist missing)..."
        Invoke-NpmBuild -Directory $SkillDir
        return
    }

    $bundleTime = (Get-Item -LiteralPath $DaemonMjs).LastWriteTime
    $staleInput = Get-NewestBuildInput -BaseDir $SkillDir -MetadataFiles @(
        'scripts/build.js',
        'package.json',
        'package-lock.json'
    )

    if ($staleInput -and $staleInput.LastWriteTime -gt $bundleTime) {
        Write-Host "Rebuilding daemon bundle (input changed: $($staleInput.FullName))..."
        Invoke-NpmBuild -Directory $SkillDir
        return
    }

    if ($coreDistNewest -and $coreDistNewest.LastWriteTime -gt $bundleTime) {
        Write-Host "Rebuilding daemon bundle (Remote Agent Control core dist changed: $($coreDistNewest.FullName))..."
        Invoke-NpmBuild -Directory $SkillDir
    }
}
...
<service>
  <id>$ServiceName</id>
  <name>Remote Agent Control Bridge</name>
  <description>Remote Agent Control bridge daemon</description>
...
function Install-NSSMService {
    ...
    & $NSSMPath set $ServiceName Description "Remote Agent Control bridge daemon"
...
switch ($Command) {
    ...
    'help' {
        Write-Host "Usage: daemon.ps1 {start|stop|restart|status|logs [N]|install-service|uninstall-service}"
        Write-Host ""
        Write-Host "Commands:"
