...
ensure_built() {
  ...
  if [ ! -f "$SKILL_DIR/dist/daemon.mjs" ]; then
    ...
    # Also check if node_modules/remote-agent-control-core was updated (npm update).
    # It stays external in the bundle, but a stale daemon should still rebuild
    # after local package changes so generated import metadata stays current.
    if [ "$need_build" = "0" ] && [ -d "$SKILL_DIR/node_modules/remote-agent-control-core/src" ]; then
      local newest_dep
      newest_dep=$(find "$SKILL_DIR/node_modules/remote-agent-control-core/src" -name '*.ts' -newer "$SKILL_DIR/dist/daemon.mjs" 2>/dev/null | head -1)
