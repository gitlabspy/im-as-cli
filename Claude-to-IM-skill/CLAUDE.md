# CLAUDE.md — Project Guidelines for remote-agent-control-skill
...
>
> ```
> 请帮我排查 remote-agent-control 桥接服务的问题。
> 1. 读取 ~/.claude-to-im/logs/bridge.log 最近 50 行日志
> 2. 读取 ~/.claude-to-im/config.env 检查配置是否正确
> 3. 运行 bash ~/.claude/skills/remote-agent-control/scripts/doctor.sh 并分析输出
> 4. 根据日志和配置给出具体的修复建议
> ```
